...
{
    public class JobDriver_ConnectSinkThenDisconnect : JobDriver
    {
        ...
        override protected IEnumerable<Toil> MakeNewToils()
        {
            ...
            connectSink.initAction = () => energySystem.AttachSink(sink);
            if(esJob.disconnectWhen.whenTag != DisconnectWhenTag.Never) {
                connectSink.defaultCompleteMode = ToilCompleteMode.Never;
                connectSink.tickAction = () => {
                    if(esJob.disconnectWhen.ShouldDisconnectSystemFrom(energySystem, sink))
            ...
            if(esJob.disconnectWhen.whenTag != DisconnectWhenTag.Never) {
                Toil disconnectSink = new Toil();
                disconnectSink.initAction = () => energySystem.DetachSink(sink);
                yield return disconnectSink;
            }
        }
    }
}
